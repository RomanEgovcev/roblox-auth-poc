"""Clean test - does the flow work today?"""
import os, time
...
with sync_playwright() as p:
    ...
    def on_req(req):
        if t0[0] is None:
            return
        if any(x in req.url for x in ("/v2/login", "pow-puzzle", "challenge/v1", "worker-resources", "account-security", "px-cloud")):
            dt = time.time() - t0[0]
            m = " **LOGIN**" if "/v2/login" in req.url else ""
            print(f"[{dt:5.1f}s] {req.method} {req.url[:120]}{m}", flush=True)
    
    def on_resp(resp):
        if t0[0] is None:
            return
        if any(x in resp.url for x in ("/challenge/v1/continue", "pow-puzzle/verify")):
            dt = time.time() - t0[0]
            try:
                b = resp.body()[:400]
                print(f"[{dt:5.1f}s] RESP {resp.status} {resp.url[:80]} body={b}", flush=True)
            except:
                print(f"[{dt:5.1f}s] RESP {resp.status} {resp.url[:80]} (no body)", flush=True)
