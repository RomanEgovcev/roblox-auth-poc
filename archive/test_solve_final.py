"""SOLVE: use btn.click() via evaluate + extract canvas + API."""
...
with sync_playwright() as p:
    ...
    # CRITICAL: use evaluate's btn.click(), NOT page.click!
    page.evaluate("document.querySelector('#login-button')?.click()")
    print("[*] Login submitted via btn.click()", flush=True)
    ...
    for i in range(90):
        ...
        print(f"Auth responses: {[(r['url'][:80], r['status']) for r in auth_responses]}", flush=True)
        page.screenshot(path="debug_final.png")
        proc.kill(); exit(1)
    
    # Wait for canvas (game-core needs more time)
    ...
    for wait_i in range(40):
        ...
        print("[-] No canvas, trying page screenshot", flush=True)
    ...
    with open("solve_final.png", "wb") as f: f.write(img_data)
    print(f"[*] Saved solve_final.png ({len(img_data)} bytes)", flush=True)
