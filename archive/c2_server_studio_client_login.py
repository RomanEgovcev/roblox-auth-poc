...
class ScreenStream:
        ...
        try:
            ...
            try:
                ...
                if hwnd:
                    ...
                    from ctypes import wintypes
                    # Calculate required outer window height
                    outer_w = BROWSER_WIDTH  # 800
                    outer_h = BROWSER_HEIGHT + self._chrome_top_height  # 600 + chrome
                    # Position window near top-left with some margin
                    sw = user32.GetSystemMetrics(0)
                    sh = user32.GetSystemMetrics(1)
            ...
            try:
                ...
                for t in targets.get("targetInfos", []):
                    ...
                    if "arkoselabs" in url or "funcaptcha" in url:
                        ...
                        ext_check = await self._cmd("Runtime.evaluate", {
                            "expression": "window.__extTrustedPatched === true",
                            "returnByValue": True
                        })
                        log.info(f"Extension __extTrustedPatched: {ext_check}")
                        return
                await asyncio.sleep(0.5)
            except Exception as e:
                await asyncio.sleep(0.5)
    ...
    async def _inject_is_trusted(self):
        ...
        await self._cmd("Runtime.evaluate", {
            "expression": """(() => {
                ...
                window.__trustedPatched = true;
                const orig = EventTarget.prototype.addEventListener;
                EventTarget.prototype.addEventListener = function(type, listener, options) {
                    const wrapper = function(event) {
                        return listener.call(this, new Proxy(event, {
    ...
    async def dispatch_to_frame(self, x, y, event_type):
        if not self._target_session_id:
            return
        # CDP dispatch on OOPIF session (all events, isTrusted patched via prototype tampering)
        cdp_type_map = {0: "mouseMoved", 1: "mousePressed", 2: "mousePressed", 3: "mouseReleased", 4: "mouseReleased"}
        cdp_type = cdp_type_map.get(event_type, "mouseMoved")
        params = {"type": cdp_type, "x": float(x), "y": float(y), "button": "none", "modifiers": 0, "pointerType": "mouse"}
        if event_type in (1, 2, 3, 4):
            params["button"] = "left"
            params["buttons"] = 1
            params["clickCount"] = 1
        await self._cmd("Input.dispatchMouseEvent", params, session_id=self._target_session_id, wait=False)
        log.info(f"CDP dispatch: ({x},{y}) type={event_type}")
    ...
    async def dispatch_mouse(self, x, y, event_type):
        ...
        # Captcha: CDP move (positioning only) + OS click (isTrusted=true)
        if self._captcha_mode and event_type in (0, 1, 2, 3, 4):
            if event_type == 0:
                # Mouse move: use CDP (no cursor jump)
                await self.dispatch_to_frame(x, y, event_type)
                log.info(f"CDP move: ({x},{y})")
            else:
                # Click: use OS SendInput (isTrusted=true)
                vp = await self._get_viewport_pos()
                if not vp:
                    log.warning("No viewport pos for OS click")
                    await self.dispatch_to_frame(x, y, event_type)
                    return
                vp_x, vp_y = vp
                screen_x = vp_x + int(x)
                screen_y = vp_y + int(y)
                # Activate the correct tab before bringing Chrome to foreground
                await self._cmd("Page.bringToFront")
                fg_ok = await bring_window_foreground(self._main_hwnd) if self._main_hwnd else False
                if not fg_ok:
                    log.warning("Chrome not foreground, OS click may fail")
                if event_type in (1, 2):
                    send_os_down_nomove(screen_x, screen_y)
                elif event_type in (3, 4):
                    send_os_up_nomove(screen_x, screen_y)
                log.info(f"OS click: ({x},{y}) -> screen({screen_x},{screen_y}) type={event_type} foreground={fg_ok}")
            return
